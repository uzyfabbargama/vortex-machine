...
class Vortex6(Vortex5):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.children = []           # Vórtices hijos (turbulencia)
        self.fractal_level = kwargs.get('fractal_level', 0)  # Nivel de recursividad
        self.max_fractal_level = 3    # Profundidad máxima de la recursividad
        
    def spawn_children(self, dt=0.1):
        """
        Crea vórtices hijos (turbulencia) a partir del vórtice actual.
        """
        # Solo crear hijos si:
        # 1. Hay suficiente energía
        # 2. No se ha alcanzado el nivel máximo de recursividad
        # 3. El vórtice está activo
        if (self.energy > 100 and 
            self.fractal_level < self.max_fractal_level and 
            self.velocity > 0):
            
            num_children = random.randint(2, 4)  # 2-4 hijos por nivel
            
            for _ in range(num_children):
                # Los hijos heredan propiedades del padre, pero con variación
                child_energy = self.energy * random.uniform(0.1, 0.3)
                child_velocity = self.velocity * random.uniform(0.3, 0.8)
                child_radius = self.radius * random.uniform(0.1, 0.3)
                child_sign = -self.sign if random.random() < 0.3 else self.sign
                
                # Crear el hijo
                child = Vortex6(
                    energy=child_energy,
                    velocity=child_velocity,
                    pressure=self.pressure * random.uniform(0.5, 0.9),
                ...
                # Posición aleatoria alrededor del padre
                angle = random.uniform(0, 2 * math.pi)
                distance = self.radius * random.uniform(0.5, 1.5)
                child.x = self.x + math.cos(angle) * distance
                child.y = self.y + math.sin(angle) * distance
                
                # El padre pierde energía al crear hijos
                self.energy -= child_energy * 0.5
                
                self.children.append(child)
    
    def update(self, dt=0.1):
        # 1. Crear hijos (si el vórtice tiene suficiente energía)
        self.spawn_children(dt)
        
        # 2. Actualizar cada hijo recursivamente
        for child in self.children:
            # Los hijos se actualizan a sí mismos (pueden crear nietos)
            child.update(dt)
            
            # Los hijos roban energía del padre (cascada de energía)
            energy_stolen = child.energy * 0.01 * dt
            self.energy -= energy_stolen
            child.energy += energy_stolen * 0.5  # El hijo retiene parte de la energía robada
        
        # 3. Eliminar hijos que se han disipado
        self.children = [c for c in self.children if c.energy > 0.1 and c.velocity > 0]
        
        # 4. Actualizar el vórtice padre
        return super().update(dt)
    
    def get_all_vortices(self):
        """
        Devuelve todos los vórtices de la jerarquía (incluyendo hijos y nietos).
        Útil para visualizar la turbulencia fractal.
        """
        all_vortices = [self]
        for child in self.children:
            all_vortices.extend(child.get_all_vortices())
        return all_vortices
